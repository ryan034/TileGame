...
private class AnimationCommand
{
    ...
    */
    ...
    private void Update()
    {
        ...
        {
            if (commandsQueue[0].path.Count > 0)
            {
                commandsQueue[0].unit.transform.position = Vector3.MoveTowards(commandsQueue[0].unit.transform.position, LocalToWord(commandsQueue[0].path[0]), animationSpeed * Time.deltaTime);
                if (Vector3.Distance(commandsQueue[0].unit.transform.position, LocalToWord(commandsQueue[0].path[0])) < 0.01f)
                {
                    commandsQueue[0].unit.transform.position = LocalToWord(commandsQueue[0].path[0]);
                    commandsQueue[0].path.RemoveAt(0);
                    if (commandsQueue[0].path.Count > 0)
                    {
                        //set rotation to upcoming command
                        commandsQueue[0].unit.transform.forward = LocalToWord(commandsQueue[0].path[0]) - commandsQueue[0].unit.transform.position;
                    }
                    else
                    {
                        commandsQueue[0].unit.Animate("Idle");
                        ...
                        else
                        {
                            ...
                            {
                                Pointer.globalInstance.HaltInput = true;
                                RotateTo(commandsQueue[0].unit, commandsQueue[0].path[0]);
                            }
                            else
                            {
                                Pointer.globalInstance.HaltInput = false;
                                RotateTo(commandsQueue[0].unit, commandsQueue[0].target);
                            }
                            commandsQueue[0].unit.Animate(commandsQueue[0].animation);
                        }
                    }
                }
            }
            ...
            //{
                ...
                //commandsQueue[0].unit.transform.rotation = Quaternion.RotateTowards(commandsQueue[0].unit.transform.rotation, q, moveSpeed * Time.deltaTime);
                
            //}
            else
            {
                if (!commandsQueue[0].unit.IsPlaying(commandsQueue[0].animation))
                {
                    //this.animator.GetCurrentAnimatorStateInfo(0).IsName("YourAnimationName")
                    // Avoid any reload.
                    commandsQueue[0].unit.Animate("Idle");
                    if (commandsQueue[0].death) { Destroy(commandsQueue[0].unit); }
                    commandsQueue.RemoveAt(0);
                    if (commandsQueue.Count == 0)
                    {
                        Pointer.globalInstance.HaltInput = false;
